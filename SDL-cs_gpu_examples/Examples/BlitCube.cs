...
public static class BlitCube
{
    public static int Main()
    {
        ...
        // Create window
        var window = SDL.CreateWindow("BlitCube", 640, 480, 0);
        ...
        // Set up texture data
        const uint bytesPerImage = 32 * 32 * 4;
        ...
        string[] imageNames = ["cube0.bmp", "cube1.bmp", "cube2.bmp", "cube3.bmp", "cube4.bmp", "cube5.bmp"];

        for (int i = 0; i < imageNames.Length; i++)
        {
            var imageData = Common.LoadImage(device, imageNames[i], out _, out _);
            ...
            unsafe
            {
                ...
                Buffer.MemoryCopy(
                    (void*)surfacePtr->Pixels,
                    (void*)(textureTransferData + (nint)(bytesPerImage * i)),
        ...
        for (int i = 0; i < 6; i++)
        {
            SDL.UploadToGPUTexture(copyPass,
                new SDL.GPUTextureTransferInfo { TransferBuffer = textureTransferBuffer, Offset = bytesPerImage * (uint)i },
                new SDL.GPUTextureRegion { Texture = sourceTexture, Layer = (uint)i, W = 32, H = 32, D = 1 },
                false);
        }

        SDL.EndGPUCopyPass(copyPass);

        // Blit to destination texture (cube->cube blit)
        for (int i = 0; i < 6; i++)
        {
        ...
        while (running)
        {
            ...
            if (swapchainTexture != IntPtr.Zero)
            {
                ...
                var texSamplerBinding = new SDL.GPUTextureSamplerBinding { Texture = destinationTexture, Sampler = sampler };
